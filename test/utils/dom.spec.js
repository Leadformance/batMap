...
test("extract text from text", () => {
    expect(dom.extractTextAndCssClasses("a content").textContent).toBe(
        "a content"
    );
});
...
test("extract text from html", () => {
    expect(
        dom.extractTextAndCssClasses("<div>a content</div>").textContent
    ).toBe("a content");
});

test("extract css class from fragment", () => {
    expect(
        dom.extractTextAndCssClasses('<div class="myClass"></div>').classes
    ).toEqual("myClass");
});

test("extract css classes from fragment", () => {
    expect(
        dom.extractTextAndCssClasses('<div class="myClass myOtherClass"></div>')
            .classes
...
test("extract style from css class", () => {
  ...
  position: absolute;
    ...
    expect(dom.getStyleFromCss("map-custom-label")).toEqual({
        left: 40,
        top: 2,
    });
});

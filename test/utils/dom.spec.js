...
test('extract text from text', () => {
  expect(domUtils.extractTextAndCssClasses('a content').textContent).toBe(
    'a content',
  );
});
...
test('extract no css class from text', () => {
  expect(
    domUtils.extractTextAndCssClasses('a content').classes,
  ).toBeUndefined();
});

test('extract text from html', () => {
  expect(
    domUtils.extractTextAndCssClasses('<div>a content</div>').textContent,
  ).toBe('a content');
});

test('extract css class from fragment', () => {
  expect(
    domUtils.extractTextAndCssClasses('<div class="myClass"></div>').classes,
  ).toEqual('myClass');
});

test('extract css classes from fragment', () => {
  expect(
    domUtils.extractTextAndCssClasses(
      '<div class="myClass myOtherClass"></div>',
    ).classes,
  ).toEqual('myClass myOtherClass');
});
...
test('extract no css class and no text from empty string', () => {
  const textAndCssClasses = domUtils.extractTextAndCssClasses('');
...
test('extract style from css class', () => {
  ...
  expect(domUtils.getStyleFromCss('map-custom-label')).toEqual({
    left: 40,
    top: 2,
  });
});
